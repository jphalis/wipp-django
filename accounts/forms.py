...
class UserCreationForm(forms.ModelForm):
    ...
    def clean_password2(self):
        """
        Verifies that the password is longer than the minimum set value,
        and makes sure the two inputted passwords match each other.
        """
...
class RegisterForm(forms.Form):
    ...
    def clean_email(self):
        """
        Verifies that the email is not already being used, and
        validates that the email ends in '.edu'.
        """
...
class AccountBasicsChangeForm(forms.ModelForm):
    full_name = forms.CharField(widget=forms.TextInput(), max_length=120,
                                label='Name')
    email = forms.EmailField(widget=forms.EmailInput(), max_length=80)
