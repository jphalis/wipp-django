...
def _apns_send(token, alert, badge=None, sound=None, category=None,
               content_available=False, action_loc_key=None, loc_key=None,
               loc_args=[], extra={}, identifier=0, expiration=None,
               priority=10, socket=None):
    data = {}
    aps_data = {}
    ...
    frame = _apns_pack_frame(token, json_data, identifier,
                             expiration_time, priority)
...
def apns_send_bulk_message(registration_ids, alert, **kwargs):
    ...
    with closing(_apns_create_socket_to_push()) as socket:
        for identifier, registration_id in enumerate(registration_ids):
            _apns_send(registration_id, alert, identifier=identifier,
