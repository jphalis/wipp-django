...
def _gcm_send(data, content_type):
    ...
    if not key:
        raise ImproperlyConfigured(
            'You need to set PUSH_NOTIFICATIONS_SETTINGS["GCM_API_KEY"] to '
            'send messages through GCM.')
...
def _gcm_send_plain(registration_id, data, **kwargs):
    ...
    # If second line starts with registration_id, gets its value and replace
    # the registration tokens in your server database. Otherwise, get the
    # value of Error
    ...
    if response["failure"] or response["canonical_ids"]:
        ...
        for index, result in enumerate(response["results"]):
            ...
            if error:
                ...
                # (https://developers.google.com/cloud-messaging/http)
                # If error is NotRegistered or InvalidRegistration, then we
                # will deactivate devices because this registration ID is no
                # more valid and can't be used to send messages, otherwise
                # raise error
            ...
            # If registration_id is set, replace the original ID with the
            # new value (canonical ID) in your server database. Note that the
            # original ID is not part of the result, so you need to obtain it
            # from the list of registration_ids passed in the request.
...
def gcm_send_message(registration_id, data, **kwargs):
    ...
    A reference of extra keyword arguments sent to the server is
    available here:
